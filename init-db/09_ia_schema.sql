...
CREATE TABLE IF NOT EXISTS ia.chat_message (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    conversation_id UUID NOT NULL REFERENCES ia.conversation(id) ON DELETE CASCADE,
...
-- Insertar 5 conversaciones de ejemplo
INSERT INTO ia.conversation (id)
SELECT gen_random_uuid()
FROM generate_series(1,5)
ON CONFLICT DO NOTHING;

-- Insertar mensajes asociados a las conversaciones
INSERT INTO ia.chat_message (conversation_id, user_message, response, raw_response)
SELECT c.id, msgs.msg, msgs.resp, jsonb_build_object('confidence', msgs.conf)
FROM ia.conversation c
CROSS JOIN LATERAL (
    VALUES
        ('¿Cuáles son los requisitos para renovar el DNI?', 'Para renovar el DNI necesitas: 1) Una foto reciente, 2) El pago de la tasa, 3) El DNI anterior.', 0.95),
        ('¿Puedo renovar si estoy en el extranjero?', 'Sí, pero deberás acudir al consulado correspondiente.', 0.9),
        ('¿Cuál es el horario de atención?', 'El horario de atención es de lunes a viernes de 09:00 a 14:00.', 0.9),
        ('¿Cómo solicito cita previa?', 'Puedes solicitarla online o por teléfono.', 0.9),
        ('¿Qué documentos necesito para empadronarme?', 'Necesitas DNI/NIE y un justificante de domicilio.', 0.9),
        ('¿Cuánto tarda en llegar un certificado?', 'Generalmente 3-5 días hábiles.', 0.9)
) AS msgs(msg, resp, conf);
