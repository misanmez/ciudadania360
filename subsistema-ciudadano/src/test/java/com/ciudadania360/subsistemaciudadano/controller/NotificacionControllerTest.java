package com.ciudadania360.subsistemaciudadano.controller;

import com.ciudadania360.subsistemaciudadano.application.dto.notificacion.NotificacionRequest;
import com.ciudadania360.subsistemaciudadano.application.dto.notificacion.NotificacionResponse;
import com.ciudadania360.subsistemaciudadano.application.service.NotificacionService;
import com.ciudadania360.shared.exception.GlobalExceptionHandler;
import com.ciudadania360.shared.exception.BadRequestException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule;
import org.junit.jupiter.api.Test;
import org.springframework.http.MediaType;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.test.web.servlet.setup.MockMvcBuilders;

import java.time.Instant;
import java.util.List;
import java.util.UUID;

import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.*;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

class NotificacionControllerTest {

    private final ObjectMapper objectMapper = new ObjectMapper()
            .registerModule(new JavaTimeModule());  // ✅ soporte para Instant

    @Test
    void listAndCrudOperations() throws Exception {
        NotificacionService service = mock(NotificacionService.class);
        UUID id = UUID.randomUUID();
        UUID solicitudId = UUID.randomUUID();

        // Notificación de ejemplo
        NotificacionResponse e = NotificacionResponse.builder()
                .id(id)
                .solicitudId(solicitudId)
                .canal("Email")
                .estado("Enviado")
                .fechaEnvio(Instant.now())
                .mensaje("Mensaje de prueba")
                .metadata("{\"prioridad\":\"alta\"}")
                .build();

        // Mock del servicio
        when(service.list()).thenReturn(List.of(e));
        when(service.get(id)).thenReturn(e);
        when(service.create(any(NotificacionRequest.class))).thenReturn(e);
        when(service.update(eq(id), any(NotificacionRequest.class))).thenReturn(e);

        // Configuración de MockMvc con GlobalExceptionHandler
        MockMvc mvc = MockMvcBuilders.standaloneSetup(new NotificacionController(service))
                .setControllerAdvice(new GlobalExceptionHandler())
                .build();

        // List
        mvc.perform(get("/api/notificaciones"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(id.toString()))
                .andExpect(jsonPath("$[0].canal").value("Email"));

        // Create válido
        NotificacionRequest createRequest = new NotificacionRequest(
                solicitudId,
                "Email",
                Instant.now(),
                "Pendiente",
                "Mensaje de prueba",
                "{\"prioridad\":\"alta\"}"
        );

        mvc.perform(post("/api/notificaciones")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(createRequest)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id").value(id.toString()))
                .andExpect(jsonPath("$.canal").value("Email"));

        // Simular error de validación
        NotificacionRequest badRequest = new NotificacionRequest(); // campos vacíos
        when(service.create(any(NotificacionRequest.class)))
                .thenThrow(new BadRequestException("SolicitudId es obligatorio"));

        mvc.perform(post("/api/notificaciones")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(badRequest)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("BAD_REQUEST"))
                .andExpect(jsonPath("$.message").value("SolicitudId es obligatorio"))
                .andExpect(header().exists("X-Request-Id"));

        // Get by ID
        mvc.perform(get("/api/notificaciones/" + id))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(id.toString()));

        // Update
        NotificacionRequest updateRequest = new NotificacionRequest(
                solicitudId,
                "SMS",
                Instant.now(),
                "Enviado",
                "Mensaje actualizado",
                "{\"prioridad\":\"media\"}"
        );

        mvc.perform(put("/api/notificaciones/" + id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateRequest)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(id.toString()));

        // Delete
        doNothing().when(service).delete(id);
        mvc.perform(delete("/api/notificaciones/" + id))
                .andExpect(status().isNoContent());
    }
}
