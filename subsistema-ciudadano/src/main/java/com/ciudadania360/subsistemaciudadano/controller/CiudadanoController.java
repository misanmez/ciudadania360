...
public class CiudadanoController {
    ...
    @PostMapping
    public ResponseEntity<CiudadanoResponse> create(@RequestBody CiudadanoRequest request) {
        CiudadanoResponse created = service.create(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }

    @PutMapping("/{id}")
    public CiudadanoResponse update(@PathVariable("id") UUID id, @RequestBody CiudadanoRequest request) {
